...
### Added
- Integrated **Note Pinning Priority & Custom Sorting Engine** in `src/lib/services/sortingService.ts` and `src/routes/+page.svelte`.
- Added 5 sorting strategies (`Pinned First`, `Recently Updated`, `Date Created`, `Title A-Z`, `Word Count High-Low`).
- Added `📊 Sort By` selector pill bar in main notebook toolbar and Command Palette sorting triggers (`⚡ ⌘K -> 📊 Sort Notes`).
- Added unit test suite in `src/lib/services/sortingService.spec.ts`.
